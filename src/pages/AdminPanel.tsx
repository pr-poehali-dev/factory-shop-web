...
const INITIAL_CLIENTS: Client[] = [
  { id: 1, company: "ООО «РадиоТех»", contact: "Иванов Иван Иванович", phone: "+7 (495) 123-45-67", email: "ivanov@radiotech.ru" },
  { id: 2, company: "АО «ЭлектроПром»", contact: "Петров Пётр Петрович", phone: "+7 (812) 234-56-78", email: "petrov@elprom.ru" },
  { id: 3, company: "ООО «СвязьМонтаж»", contact: "Сидорова Анна Сергеевна", phone: "+7 (343) 345-67-89", email: "sidorova@svyaz.ru" },
  { id: 4, company: "ИП Кузнецов А.В.", contact: "Кузнецов Алексей Викторович", phone: "+7 (903) 456-78-90", email: "kuznetsov@mail.ru" },
];

const INITIAL_ORDERS: Order[] = [
  { id: 1001, clientId: 1, date: "2024-05-08", status: "completed", amount: 45600 },
  { id: 1002, clientId: 2, date: "2024-05-09", status: "shipped", amount: 128400 },
  { id: 1003, clientId: 3, date: "2024-05-09", status: "processing", amount: 32100 },
  { id: 1004, clientId: 1, date: "2024-05-10", status: "new", amount: 78900 },
  { id: 1005, clientId: 4, date: "2024-05-10", status: "new", amount: 15200 },
];

const INITIAL_PRODUCTS: AdminProduct[] = [
  { id: 1, name: "Транзистор КТ815А", article: "КТ815А", category: "Транзисторы", price: 18, stock: 5400 },
  { id: 2, name: "Транзистор КТ3102", article: "КТ3102", category: "Транзисторы", price: 12, stock: 8200 },
  { id: 3, name: "Диод КД522Б", article: "КД522Б", category: "Диоды", price: 6, stock: 12400 },
  { id: 4, name: "НЧ соединитель РП15", article: "РП15-9", category: "НЧ соединители", price: 245, stock: 320 },
  { id: 5, name: "ВЧ кабель РК50", article: "РК50-2-21", category: "ВЧ кабельные сборки", price: 480, stock: 150 },
];
...
export default function AdminPanel({ user, onLogout }: AdminPanelProps) {
  ...
  const deleteClient = (id: number) => {
    if (confirm("Удалить клиента?")) setClients((p) => p.filter((c) => c.id !== id));
  };
  const deleteProduct = (id: number) => {
    if (confirm("Удалить товар?")) setProducts((p) => p.filter((c) => c.id !== id));
  };
  const deleteOrder = (id: number) => {
    if (confirm("Удалить заказ?")) setOrders((p) => p.filter((c) => c.id !== id));
  };
  ...
  const saveClient = (c: Client) => {
    if (clients.find((x) => x.id === c.id)) {
      ...
      setClients((p) => [...p, { ...c, id: Math.max(0, ...p.map((x) => x.id)) + 1 }]);
    }
                      ...
                      <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="inline-flex gap-1">
                            <button onClick={() => setEditingClient(c)} className="p-1.5 hover:bg-blue-100 text-[#1a9fd4] rounded" title="Редактировать">
                              <Icon name="Pencil" size={14} />
                            </button>
                            <button onClick={() => deleteClient(c.id)} className="p-1.5 hover:bg-red-100 text-red-500 rounded" title="Удалить">
                  ...
                  <tbody>
                    {orders.map((o) => (
                      <tr key={o.id} className="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-700 font-mono text-xs">{new Date(o.date).toLocaleDateString("ru-RU")}</td>
                        <td className="px-4 py-3">
                          <select
                            value={o.status}
                            onChange={(e) => setOrders((p) => p.map((x) => (x.id === o.id ? { ...x, status: e.target.value as Order["status"] } : x)))}
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="inline-flex gap-1">
                            <button className="p-1.5 hover:bg-blue-100 text-[#1a9fd4] rounded" title="Просмотр">
                              <Icon name="Eye" size={14} />
                            </button>
                            <button onClick={() => deleteOrder(o.id)} className="p-1.5 hover:bg-red-100 text-red-500 rounded" title="Удалить">
                              <Icon name="Trash2" size={14} />
                            </button>
                          </div>
                        </td>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  { label: "Всего заказов", value: totalOrders, icon: "ClipboardList", color: "text-blue-600", bg: "bg-blue-50" },
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 pt-2">
        <button type="button" onClick={onCancel} className="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2 rounded-lg text-sm">
          Отмена
        </button>
        <button type="submit" className="flex-1 bg-[#1a9fd4] hover:bg-[#158ab8] text-white font-medium py-2 rounded-lg text-sm">
          Сохранить
        </button>
      </div>
    </form>
